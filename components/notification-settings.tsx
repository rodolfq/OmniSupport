    ...
    <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-300">
       <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div className="space-y-6">
            <div className="p-6 bg-slate-50 dark:bg-[var(--surface-card)] rounded-2xl border border-slate-100 dark:border-[var(--border-default)] flex flex-col gap-4">
               <h4 className="text-[10px] font-black uppercase text-slate-800 dark:text-[var(--text-primary)] tracking-widest flex items-center gap-2">
                 <Volume2 size={14} className="text-indigo-600 dark:text-[var(--accent-text)]" /> Sons do Sistema
               </h4>
               
               <div className="space-y-4">
                  <div className="space-y-1">
                     <label className="text-[9px] font-black uppercase text-slate-400 dark:text-[var(--text-tertiary)] tracking-widest">Alerta de Sistema</label>
                     <div className="flex gap-2">
                        <StyledSelect 
                          value={notificationSettings.systemSound}
                          onChange={(e) => updateNotificationSettings({ systemSound: e.target.value })}
                          className="flex-1 bg-white dark:bg-[var(--surface-card)] border border-slate-200 dark:border-[var(--border-default)] rounded-xl px-3 py-2 text-xs font-bold outline-none focus:border-indigo-500 dark:focus:border-[var(--accent)]"
                        >
                        ...
                        <button 
                          ...
                          className="p-2 bg-indigo-100 dark:bg-[var(--accent)]/20 text-indigo-600 dark:text-[var(--accent-text)] rounded-xl hover:bg-indigo-200 transition-all"
                        >
                           <Volume2 size={16} />
                        </button>
                     </div>
                  ...
                  <div className="space-y-1">
                     <label className="text-[9px] font-black uppercase text-slate-400 dark:text-[var(--text-tertiary)] tracking-widest">Alerta de Chat</label>
                     <div className="flex gap-2">
                        <StyledSelect 
                        ...
                        <button 
                          ...
                          className="p-2 bg-emerald-100 dark:bg-[var(--surface-success)] text-emerald-600 dark:text-[var(--text-success)] rounded-xl hover:bg-emerald-200 transition-all"
                        >
                           <Volume2 size={16} />
                        </button>
                     </div>
          ...
          <div className="space-y-4">
             <h4 className="text-[10px] font-black uppercase text-slate-800 dark:text-[var(--text-primary)] tracking-widest px-2">Notificações Ativas</h4>
             <div className="bg-white dark:bg-[var(--surface-card)] border border-slate-100 dark:border-[var(--border-default)] rounded-2xl shadow-sm divide-y divide-slate-100 dark:divide-[var(--border-default)]">
...
function NotificationToggle({ label, active, onChange }: { label: string, active: boolean, onChange: (v: boolean) => void }) {
  return (
    <div className="flex items-center justify-between px-6 py-4 hover:bg-slate-50 dark:hover:bg-[var(--surface-card)] transition-colors">
       ...
       <button 
         ...
         className={cn(
           ...
           active ? "bg-emerald-500 dark:bg-[var(--text-success)] justify-end" : "bg-slate-200 dark:bg-[var(--border-default)] justify-start"
         )}
       >
          <div className="w-4 h-4 bg-white dark:bg-[var(--surface-card)] rounded-full shadow-sm" />
       </button>
    </div>
  );
}
