...
- **Docker** (`Dockerfile` + `docker-compose.yml`): build multi-stage sobre `node:22-bookworm-slim` (**não** Alpine — `onnxruntime-node`/`sharp`/`ffmpeg-static` são nativos contra glibc e falham em musl só em runtime), `output: 'standalone'` no `next.config.ts`, healthcheck em `GET /api/health` (rota pública em `middleware.ts`). O Postgres **não** está no compose: é o banco próprio da equipe, apontado por `DATABASE_URL`. Volumes: `attachments` (`/data/attachments`) e `models` (`/data/models`, só cache).
  - ⚠️ **A IMAGEM NUNCA FOI CONSTRUÍDA** (situação em 2026-08-12). O `Dockerfile`/`docker-compose.yml` foram escritos e revisados, mas `docker build` jamais rodou: a máquina onde foram criados está sem virtualização habilitada. O teste está previsto para outro computador. **Trate o primeiro `docker compose build` como não confiável e espere corrigir coisas.** O que ESTÁ verificado: `npm run build` do zero passa e gera `.next/standalone/server.js` com as rotas novas; `tsc --noEmit`/lint/`check:encoding` limpos; `package-lock.json` contém `@img/sharp-linux-x64` e `@img/sharp-libvips-linux-x64` (ou seja, `npm ci` resolve `sharp` no Linux — o modo de falha clássico de lockfile gerado no Windows não se aplica); `pg` está no `node_modules` do standalone e o `package.json` gerado é CommonJS, então os scripts em `scripts/` rodam dentro do container. O que só o build revela: (1) o `postinstall` do `ffmpeg-static` baixa binário da internet, então `npm ci` exige rede no build; (2) permissão dos volumes — a imagem faz `chown node:node /data`, o que cobre volume novo, mas um volume `attachments` preexistente com dono `root` deixa o container subir sem conseguir gravar anexo (só aparece quando alguém anexa arquivo); (3) o tracing do `output: 'standalone'` foi exercitado no Windows, não no Linux. Todos falham no build ou no start, ruidosamente — não há cenário de imagem subir silenciosamente errada e corromper dado.
  - **Uma réplica só, obrigatoriamente**: `lib/chat-events.ts` faz fan-out do SSE por `EventEmitter` em memória do processo e `instrumentation-node.ts` sobe 4 schedulers em todo processo que inicia — dois containers duplicariam mensagem automática enviada ao cliente. Escalar exige antes trocar o fan-out por `pg LISTEN/NOTIFY` e eleger um dono dos schedulers.
