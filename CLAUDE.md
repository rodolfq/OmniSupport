...
- **`config_email_settings`** — linha única (`id = 1`, `CHECK`), SMTP para disparo de e-mail: `enabled`, `smtp_host/port/secure/user/password`, `from_name/email`. Editável em Configurações > E-mail, sem precisar de deploy/env var — ver `lib/services/email-service.ts`.
...
- **Só o WhatsApp do Pyvon (e o widget) é distribuído automaticamente.** WhatsApp **não oficial** (QR Code / Baileys) não atende ninguém: toda mensagem recebida por ele — de contato novo **ou de quem já tinha conversa aberta com analista** — não cria conversa, não é gravada, não aciona analista nem aparece no widget; o contato só recebe o aviso de mudança de número (`lib/services/baileys-redirect.ts`, texto fixo, no máximo 1 a cada 5 min por contato; a trava é só em memória — um reinício do servidor zera — e uma tentativa que falhou também conta). A única exceção é a resposta "1"/"0" à pesquisa de satisfação de uma conversa recém-fechada, que continua sendo registrada. Conversas do QR Code que já estavam abertas ficam só para o analista fechar: o cliente delas não consegue mais responder por ali. O envio pelo analista (`WhatsAppService.sendMessage`) continua funcionando. `dispatchPendingChatSessions` ignora `channel = 'whatsapp_baileys'`. Decisão do usuário, 2026-09-24/25 — objetivo é empurrar o cliente pro número novo; a 1ª versão poupava as conversas já atribuídas e o usuário apontou que "todos os chats" devem receber o aviso. Roda só no processo que mantém o Baileys (`SERVICE_ROLE` backend/monolito), nunca no container `frontend` do dev. **Armadilha (2026-09-25):** o **Modo de Crise** (`config_crisis_mode`) mandava este mesmo texto quando a conversa nascia, então quem ainda roda o código antigo parece "funcionar" no aviso mas continua criando e atribuindo a conversa — confira em `chat_messages.metadata->>'source' = 'crisis_mode'` em sessões `whatsapp_baileys` novas: enquanto aparecerem, o processo do Baileys em produção NÃO está com a versão nova.
- Atribuição manual: campo `assignee_id` em `tickets`/`chat_sessions`, alterável pela UI (`components/assign-chat-menu.tsx`). A rota `assign` só aceita responsável de papel de equipe (Administrador/Equipe/Time Interno).
...
| `/api/email/send` | POST | Envia e-mail usando o SMTP configurado em `config_email_settings` (Configurações > E-mail) | Sessão válida |
| `/api/email/test` | POST | Dispara o e-mail de teste do botão em Configurações > E-mail | Sessão válida / admin |
...
| E-mail (SMTP) | Implementado (achado desatualizado nesta tabela em 2026-09-08 — a linha dizia "não implementado"). Configuração fica no **banco** (`config_email_settings`, linha única id=1), editável a qualquer momento em Configurações > E-mail, sem precisar de deploy — inclui botão de e-mail de teste. Falha graciosamente (erro claro + `status: failed` no log de disparo) quando desligado/incompleto, não derruba a automação | `lib/services/email-service.ts`, `lib/email-templates.ts`, `components/email-settings-content.tsx`, `app/api/email/send/route.ts`, `app/api/email/test/route.ts` |
...
| ~~Endpoint `GET /api/tickets?action=teams`~~ | **Resolvido** (achado em 2026-09-08 durante auditoria pré-operação: o código atual já deriva `member_ids` via `profiles.internal_team_ids` com JOIN, não consulta mais a coluna inexistente — corrigido em algum commit sem atualizar esta linha) | `app/api/tickets/route.ts` |
| Mensagens automáticas por instância errada | `lib/services/automation-service.ts:305` usa `WhatsAppService.sendMessage('default', ...)` fixo, em vez de resolver pela Fila do chamado (como o botão manual e o chat fazem) — pode enviar pela instância errada se houver mais de uma configurada. **Agora com impacto real**: desde a integração Pyvon (commit `b7d60ec`) existem 3 instâncias configuradas (`default`, `pyvon`, `pyvon-dev`), não mais uma só | `lib/services/automation-service.ts` |
| ~~Configuração de e-mail (SMTP)~~ | **Resolvido** (achado em 2026-09-08: já implementado, não "não iniciado" como esta linha dizia). Tela em Configurações > E-mail, configuração no banco (`config_email_settings`), edição sem deploy | `components/email-settings-content.tsx`, `lib/services/email-service.ts` |
| ~~Disparo de e-mail (resposta ao cliente + notificação de atribuição)~~ | **Resolvido** — `automation-service.ts`/`automation-scheduler.ts` já chamam `EmailService.send` para atribuição, mudança de status e outros eventos configuráveis | `lib/services/automation-service.ts`, `lib/services/automation-scheduler.ts`, `lib/services/internal-ticket-notifications.ts` |
...
1. ~~`app/api/tickets/route.ts` (`?action=teams`)~~ — **resolvido** (achado em 2026-09-08: já consulta `profiles.internal_team_ids` via JOIN, não a coluna inexistente).
2. **`lib/services/automation-service.ts:305`** — mensagens automáticas disparam sempre pela instância `'default'` do WhatsApp, ignorando a Fila do chamado. Pode sair pela instância errada com múltiplas instâncias configuradas — **desde a integração Pyvon (`b7d60ec`, 2026-09) isso deixou de ser hipotético**: há 3 instâncias reais configuradas (`default`, `pyvon`, `pyvon-dev`). Ainda não corrigido.
...
11. ~~`GET /api/tickets` sem escopo de empresa~~ — **resolvido em 2026-09-06** (commit `043829d`): a listagem, o chamado único (`?id=`) e as mensagens (`?action=messages`) não filtravam por `company_id` — qualquer sessão autenticada de Cliente/Funcionário lia chamado de qualquer empresa (o filtro existia só no client, em `app/(portal)/my-tickets/page.tsx`). A criação (`action=create`) também confiava no `companyId` enviado pelo corpo da requisição em vez do `company_id` do próprio perfil. Achado montando a suíte de testes abaixo — ver `tests/api/tickets-api.spec.ts`.
12. ~~Hook condicional em `ticket-detail-modal.tsx`~~ — **resolvido em 2026-09-08**: `useAutoTranscribeMissingAudio` era chamado depois de um `if (!ticket) return null;`, violando rules-of-hooks (quantidade de hooks variava entre renders). Só não pegava no build por causa do item 3 acima (lint não bloqueia). Movido para antes do retorno condicional.
13. **Suíte de testes automatizados** (`tests/`, Playwright) adicionada em 2026-09-06 — cobre login por papel, criação de chamado e isolamento entre empresas. Roda contra um banco descartável próprio (`ssx_test` dentro do container `ssx-pg-dev`), nunca o banco real. Ver `tests/README.md` para como rodar (`npm run test:e2e`) e a arquitetura (por que 1 worker só, por que os projetos do Playwright têm `dependencies` entre si). Não está em nenhum CI ainda — só roda quando alguém dispara manualmente.
...
- **Citar mensagem no Pyvon ("responder" do WhatsApp) — `reply_to_message_id`, contrato v1.14.** Só o canal Pyvon, só pra equipe. O id que o Pyvon entende é o `chat_messages.pyvon_message_id`: as mensagens do CLIENTE já chegam com ele pelo webhook, e as NOSSAS passam a ter depois do envio (`app/api/whatsapp/send/route.ts` grava o `message_id` que o `bot-response` devolve) — mensagem nossa enviada antes dessa mudança não tem id e não pode ser citada (a listagem devolve `pyvonQuotable`, e o botão "Responder" só aparece quando é true). O widget manda o id NOSSO da mensagem citada; o servidor resolve o id do Pyvon da MESMA conversa e monta o trecho gravado em `metadata.replyTo` a partir do banco (`lib/services/chat-reply.ts`) — nunca aceita o `replyTo` que vem do navegador. Se o Pyvon recusar a citação (422: mensagem não citável, ex.: ainda não entregue), a resposta é reenviada SEM citar em vez de se perder, e o widget avisa. Citação feita pelo CLIENTE (campo `quoted` do webhook de entrada, contrato v1.15): `PyvonService.resolveInboundQuote` procura a mensagem citada pelo `pyvon_message_id` NA MESMA conversa e, achando, grava `metadata.replyTo` a partir do banco (com vínculo: clicar leva até ela); não achando (ex.: template/mensagem do bot, ou conversa anterior), grava o trecho que veio no payload, sem `messageId` — aí só mostra o trecho, sem link.
- **Respostas prontas (macros) = `quick_notes`, e o botão substituiu o "só por comando"** (2026-09-25): no chat widget, o botão ao lado do microfone abre `components/quick-replies-panel.tsx` — lista TODAS as respostas com o conteúdo, busca por texto (cada palavra digitada precisa aparecer no conteúdo/título/categoria, sem acento nem maiúscula) e, ao escolher (clique ou setas+Enter), COLA o texto no campo de digitação sem enviar (com texto já digitado, entra em nova linha). O antigo comando "/atalho" foi APOSENTADO (2026-09-25): digitar "/" no campo não abre mais nada; cadastrar: botão "Nova" no próprio painel (título opcional — em branco usa o começo do texto; título repetido ganha " (2)"), ou editar/excluir em Chat > Central de Atendimento > Respostas Prontas (antes "Notas Rápidas"). `quick_notes.shortcut` é único e funciona como título; as 62 respostas importadas de `respostas_prontas.xls` (`migrations/quick_notes_respostas_prontas.sql`, aplicada em produção em 2026-09-25, só a coluna "Texto da resposta predefinida") têm como título o começo do próprio texto e `category = 'Respostas prontas'` — pra desfazer: `DELETE FROM public.quick_notes WHERE category = 'Respostas prontas'`.
- **Template automático NÃO cria conversa — só a resposta do cliente cria** (2026-09-25): chamado aberto, nota no chamado e mensagem automática fora da janela de 24h, quando o contato NÃO tem conversa aberta, ficam guardados em `pyvon_pending_outbound` (`PyvonService.recordOutboundTemplateMessage` com `deferUntilReply`). Quando o cliente responde (`handleWebhook`), a conversa nasce já atribuída ao AUTOR da nota / de quem abriu o chamado (se ele estiver online na fila; senão, rodízio), os templates entram nela como histórico (com a hora em que foram enviados) e a nota/autor viram os pendentes — então "Prosseguir" envia a nota. Janela de 48h: pendente mais velho é descartado. Com conversa já aberta, o template é registrado nela como sempre. Envio manual por analista e "iniciar conversa" continuam criando conversa. Antes, o envio do template abria uma conversa 'active' sorteada pelo rodízio; o analista sorteado fechava (perdendo a nota) ou repassava. `adoptPendingNoteFromClosedSession` cobre o caso de a conversa da nota já existir e ser fechada antes da resposta. **Postgres de produção é antigo: sem `gen_random_uuid()`** — use o padrão `md5(random()::text || clock_timestamp()::text)::uuid` nas migrations.
- **Resposta "1"/"0" da pesquisa no Pyvon (2026-09-25):** `PyvonService.tryRecordSurveyAnswer` roda ANTES de achar/criar conversa no webhook: se o contato NÃO tem conversa aberta e existe uma encerrada com `awaiting_survey_until` ainda no futuro, a resposta é gravada NELA (`metadata.survey_response`), a nota vai pra `chat_histories.rating` (1 → 1; 0 → -1) e a janela é encerrada — sem abrir atendimento novo. Só vale texto exatamente "0"/"1" (aceita ponto/espaço no fim); "1 obrigado", conversa aberta ou janela vencida seguem o fluxo normal. Antes, o Pyvon não tratava isso (só o Baileys): cada avaliação virava um chat novo e a nota nunca era gravada.
- **Registro RÍGIDO de criação de usuário (2026-09-25)** — `public.user_creation_log` (`migrations/user_creation_log.sql`): um GATILHO em `profiles` grava, na MESMA transação do INSERT, quem foi criado e por qual login (nome/e-mail/papel/empresa de ambos como FOTO, origem, IP, navegador). A tabela é somente-inserção (gatilhos barram UPDATE/DELETE/TRUNCATE) e não tem FK, então sobrevive à exclusão/edição do usuário. Se o registro falhar, a criação falha junto. O autor vem de variáveis de sessão `app.*` que `withCreationContext`/`applyCreationContext` (`lib/db.ts`) definem DENTRO da transação (`set_config(..., true)`, não vazam entre requisições); nome/papel/empresa do autor são lidos do banco pelo gatilho, não informados pela aplicação. **Ao criar um caminho novo que insere em `profiles`, use `withCreationContext`** — um INSERT fora dele ainda é registrado, mas como `source = 'desconhecido'` e sem autor. Origens em uso: `portal-cliente`, `portal-equipe`, `chat-vincular`, `cadastro-empresa`, `api-integracao`, `sincronizacao-planilha-cs`, `sincronizacao-bitrix24`, `chamado-perfil-automatico`. Consulta: Configurações > Sistema > "Criação de Usuários" (`components/user-creation-log-content.tsx`, API `/api/reports/user-creation-log`; busca + filtro por origem; mesma restrição do Log de Alterações: Administrador, `reports:audit_log` ou `settings:system`). **Qualquer usuário de empresa-cliente (Cliente OU Funcionário) cria Funcionário da própria empresa** (`app/api/users/route.ts`, `create-full`) — antes só o Cliente dono da conta e o Funcionário "Admin Cliente"; segue sem poder criar outro papel, outra empresa nem `view_all_company_tickets`. "Vincular contato" no chat (`action=create`) aceita e-mail opcional (vazio vira NULL; formato e duplicidade validados).
- **Cursor do rodízio (2026-09-25):** `pickNextQueueAssignee` descobria "quem recebeu por último" só pela conversa CRIADA mais recentemente; devolver uma conversa ANTIGA pra fila não mexe nisso, então devoluções em sequência recalculavam o mesmo ponteiro e caíam TODAS no mesmo analista (17h: as 4 conversas do Mauro foram pra Bianca — ponteiro Pedro, próxima depois dele = Bianca). Agora `queue_rotation_cursor` (`migrations/queue_rotation_cursor.sql`) guarda cada escolha do rodízio e o ponteiro é o mais RECENTE entre o cursor e a última conversa criada (`resolveRotationPointer`). Chave `'combined'` = pool do widget. Se a tabela não existir, cai no critério antigo sem quebrar. Manual ("Assumir"/"Transferir") NÃO move o cursor, só conversa nova criada depois move (como antes).
