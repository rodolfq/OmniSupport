...
| `GOOGLE_REDIRECT_URI` | URL de callback explícita, sobrepõe a dedução automática (protocolo+host da própria requisição) | Não (default: deriva da requisição) | Só necessário se front e back estiverem em domínios diferentes (ver `IS_SPLIT_DEPLOYMENT` em `lib/runtime-config.ts`) e a dedução automática não bater com o domínio público correto |
