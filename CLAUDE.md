...
- **Só o WhatsApp do Pyvon (e o widget) é distribuído automaticamente.** WhatsApp **não oficial** (QR Code / Baileys) não cria conversa nem grava mensagem para quem não tem um analista responsável: o contato só recebe o aviso de mudança de número (`lib/services/baileys-redirect.ts`, texto fixo, no máximo 1 a cada 5 min por contato; a trava é só em memória — um reinício do servidor zera — e uma tentativa que falhou também conta). Nada aparece no widget. Só segue o fluxo normal uma conversa que já está atribuída a um analista (que ele iniciou, ou já estava em curso); a mídia de quem não é atendido nem chega a ser baixada. `dispatchPendingChatSessions` ignora `channel = 'whatsapp_baileys'`. Decisão do usuário, 2026-09-24 — objetivo é empurrar o cliente pro número novo. Roda só no processo que mantém o Baileys (`SERVICE_ROLE` backend/monolito), nunca no container `frontend` do dev.
