    ...
    <div className="max-w-[1400px] mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-[var(--surface-card)] p-8 rounded-[2.5rem] border border-[var(--border-default)] shadow-sm">
        <div className="space-y-1">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-black text-[var(--text-primary)] tracking-tight">Níveis de Acesso</h2>
          </div>
          <p className="text-[var(--text-tertiary)] font-medium text-sm ml-13">Configure o que cada perfil pode ver e fazer na plataforma</p>
        </div>
        ...
        <div className="flex items-center gap-3">
          {hasChanges && (
            <button
              onClick={handleReset}
              disabled={isSaving}
              className="px-6 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] hover:bg-[var(--surface-card)] transition-all flex items-center gap-2"
            >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 sticky top-8">
        ...
        <div className="lg:col-span-3 space-y-6">
          <div className="bg-[var(--surface-card)] p-6 rounded-[2.5rem] border border-[var(--border-default)] shadow-sm space-y-6">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-black uppercase text-[var(--text-tertiary)] tracking-widest">Perfis & Permissões</span>
              <button
                onClick={() => setIsAddingRole(true)}
                className="w-8 h-8 bg-[var(--accent)]/10 text-[var(--accent-text)] rounded-xl flex items-center justify-center hover:bg-[var(--accent)]/20 transition-all"
              >
                <Plus size={18} />
              </button>
            </div>
                ...
                {isAddingRole && (
                  ...
                  >
                    ...
                    <div className="flex gap-2">
                      <button onClick={handleAddRole} className="flex-1 bg-[var(--accent)] text-white rounded-lg py-2 text-[10px] font-black uppercase">Criar</button>
                      <button onClick={() => setIsAddingRole(false)} className="px-4 py-2 text-[var(--text-tertiary)] hover:bg-[var(--surface-card)] rounded-lg transition-all"><X size={16} /></button>
                    </div>
                  </motion.div>
                )}
              </AnimatePresence>
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <div className={cn(
                        ...
                        selectedRoleId === rp.name ? "bg-white/20 text-white" : "bg-[var(--surface-pill)] text-[var(--text-tertiary)]"
                      )}>
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className={cn(
                          ...
                          selectedRoleId === rp.name ? "text-indigo-100 dark:text-[var(--accent-soft-text)]" : "text-[var(--text-tertiary)]"
                        )}>
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        selectedRoleId === rp.name
                          ...
                          : "text-slate-300 hover:text-[var(--text-danger)] hover:bg-[var(--surface-danger)]"
                      )}
                    >
                      <Trash2 size={16} />
                    </button>
        ...
        <div className="lg:col-span-9 space-y-6">
          {!currentRole ? (
            <div className="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[3rem] p-20 flex flex-col items-center justify-center text-center space-y-4">
               ...
               <h3 className="text-xl font-black text-[var(--text-primary)]">Selecione um Perfil</h3>
               <p className="text-[var(--text-tertiary)] max-w-xs">Escolha um perfil na lateral para gerenciar suas permissões de acesso.</p>
            </div>
          ) : (
              ...
              <div className="bg-[var(--surface-card)] p-6 rounded-[2.5rem] border border-[var(--border-default)] shadow-sm flex flex-col md:flex-row md:items-center gap-6">
                <div className="flex-1 space-y-1">
                  {selectedRoleId === 'Administrador' ? (
                    <div className="flex items-center gap-3">
                       <h3 className="text-lg font-black text-[var(--text-primary)] uppercase tracking-tight">{currentRole.name}</h3>
                  ...
                  <p className="text-xs text-[var(--text-tertiary)] font-medium">Configurando {currentRole.permissions.length} acessos ativos</p>
                </div>
                  ...
                  {filteredGroups.map((group) => {
                    ...
                    return (
                      <motion.div 
                        ...
                        className="bg-[var(--surface-card)] border border-[var(--border-default)] rounded-[2.5rem] shadow-sm flex flex-col overflow-hidden"
                      >
                          ...
                          {selectedRoleId !== 'Administrador' && (
                            <button
                              ...
                              className={cn(
                                ...
                                allEnabled ? "text-[var(--text-danger)] hover:text-[var(--text-danger)]" : "text-[var(--accent-text)] hover:text-[var(--accent-hover)]"
                              )}
                            >
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  ...
                                  isEnabled
                                    ? "bg-[var(--surface-card)] border-[var(--accent)]/20 hover:border-[var(--accent)]/30"
                                    : "bg-[var(--surface-card)]/50 border-transparent hover:bg-[var(--surface-card)]"
                                )}
                              >
                                <div className={cn(
                                  "mt-0.5 w-10 h-10 rounded-2xl flex items-center justify-center transition-all",
                                  isEnabled ? "bg-[var(--accent)] text-white shadow-xl shadow-indigo-100" : "bg-[var(--surface-card)] border border-[var(--border-default)] text-slate-300"
                                )}>
                                ...
                                <div className="flex-1 pr-6">
                                  <p className={cn(
                                    ...
                                    isEnabled ? "text-[var(--text-primary)]" : "text-[var(--text-tertiary)] group-hover:text-[var(--text-secondary)]"
                                  )}>
                                    {perm.label}
                                  </p>
                                  <p className="text-[10px] text-[var(--text-tertiary)] font-medium leading-normal">
                                    {perm.desc}
                                  </p>
                                </div>
        ...
        {roleToDelete && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
            ...
            <motion.div
              ...
              className="relative bg-[var(--surface-card)] w-full max-w-md rounded-[2.5rem] shadow-2xl p-10 overflow-hidden"
            >
              <div className="space-y-6 text-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-[var(--text-tertiary)] font-medium text-sm leading-relaxed">
                    Você está prestes a excluir o perfil <span className="font-bold text-[var(--text-primary)]">&quot;{roleToDelete.name}&quot;</span>.
                ...
                <div className="flex gap-4 pt-2">
                  <button
                    onClick={() => setRoleToDelete(null)}
                    className="flex-1 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-[var(--text-tertiary)] hover:text-[var(--text-secondary)] hover:bg-[var(--surface-card)] transition-all border border-[var(--border-default)]"
                  >
                    Cancelar
                  </button>
                  <button
                    ...
                    className="flex-1 px-6 py-4 rounded-2xl text-[10px] font-black uppercase tracking-widest bg-[var(--text-danger)] text-white shadow-xl shadow-red-100 hover:bg-red-700 transition-all"
                  >
                    Sim, Excluir
                  </button>
                </div>
