...
<div className="flex items-center gap-4">
            {/* Logo da empresa-cliente (ver customers/page.tsx pra troca) —
                só faz sentido pra quem é da empresa (Cliente/Funcionário);
                pra equipe interna o header continua só com o título, já que
                não há "uma" empresa associada à sessão. */}
            {[UserRole.CUSTOMER, UserRole.EMPLOYEE].includes(currentUser?.role as UserRole) && currentUser?.companyLogoThumbUrl && (
              <>
                <img
                  src={currentUser.companyLogoThumbUrl}
                  alt="Logo da empresa"
                  className="h-9 max-w-[7rem] object-contain"
                />
                <div className="h-4 w-px bg-[var(--border-default)]"></div>
              </>
            )}
            <h1 className="text-lg font-semibold text-[var(--text-primary)] tracking-tight">Portal SSX Desk</h1>
            <div className="h-4 w-px bg-[var(--border-default)]"></div>
              ...
              <div className="relative">
                ...
                {isNotificationsOpen && (
                  <>
                    ...
                    />
                    <div className="absolute right-0 mt-2 w-80 max-h-[520px] bg-[var(--surface-card)] border border-[var(--border-default)] rounded-2xl shadow-2xl z-[120] overflow-hidden flex flex-col transform origin-top-right transition-all animate-in fade-in zoom-in-95 duration-200">
                      <NotificationPanel
