  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-3xl font-black text-slate-800 uppercase tracking-tight">Histórico de Conversas</h2>
          <p className="text-slate-500 font-medium mt-1">Acesse todas as conversas finalizadas</p>
        </div>
      </div>
       ...
       <div className="flex flex-col md:flex-row gap-4">
         <div className="relative flex-1">
           ...
           <input 
             ...
             className="w-full bg-white border border-slate-200 rounded-2xl py-3 pl-12 pr-4 text-sm font-bold focus:ring-4 focus:ring-indigo-500/10 outline-none"
           />
         </div>
           ...
           {[
             ...
             <button 
               ...
               className={cn(
                 ...
                 dateFilter === opt.value ? "bg-indigo-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
               )}
             >
               {opt.label}
             </button>
         ...
         <div className="flex items-center gap-2">
           ...
           {[
             ...
             <button 
               ...
               className={cn(
                 "px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all",
                 ratingFilter === opt.value ? "bg-emerald-600 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"
               )}
             >
               {opt.label}
             </button>
       ...
       <div className="space-y-4">
         {filteredHistories.length === 0 ? (
           <div className="text-center py-20">
             ...
             <p className="text-slate-500 font-medium">Nenhuma conversa encontrada</p>
           </div>
         ) : filteredHistories.map(h => (
           <div key={h.id} className="bg-white rounded-2xl p-6 border border-slate-200 shadow-sm hover:shadow-md transition-shadow">
             <div className="flex items-start justify-between mb-4">
               <div>
                 <h3 className="text-lg font-black text-slate-800 uppercase tracking-tight">{h.customerName || 'Cliente'}</h3>
                 <p className="text-xs text-slate-500">{h.customerPhone || 'Sem telefone'}</p>
                 {h.assigneeName && (
<p className="text-[10px] text-indigo-600 font-medium mt-1">
               ...
               <div className="flex items-center gap-2">
                 {h.rating === 1 && <ThumbsUp className="text-emerald-500" size={20} />}
                 {h.rating === -1 && <ThumbsDown className="text-red-500" size={20} />}
            ...
            <div className="bg-slate-50 rounded-xl p-4 max-h-48 overflow-y-auto">
              <pre className="text-[10px] text-slate-600 whitespace-pre-wrap font-mono">{h.transcript}</pre>
            </div>
          </div>
        ))}
      </div>
