...
interface TicketKanbanStatusMeta {
  value: string;
  label: string;
  color: string;
  dot: string;
...
// Mesmo vocabulário de cor do Kanban em Tickets Internos — o anel do chip
...
export function TicketsView({
  ...
  const [kanbanStatuses, setKanbanStatuses] = useState<TicketKanbanStatusMeta[]>(DEFAULT_TICKET_KANBAN_STATUSES);
  ...
  useEffect(() => {
    async function loadKanbanStatuses() {
      try {
        ...
        console.error('Error loading ticket statuses for kanban:', error);
      }
    }
    loadKanbanStatuses();
  }, []);
  ...
  // Muda o status de UM chamado só — usado pelo drag-and-drop do Kanban
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between gap-2 mb-3">
          <div className="flex items-center gap-2">
            <span className="font-mono text-[10px] font-semibold text-[var(--accent-text)] bg-[var(--accent)]/10 px-2 py-0.5 rounded">
              #{t.ticketNumber ? String(t.ticketNumber).padStart(4, '0') : t.id.slice(0, 8)}
            </span>
            {chip && (
              <span className="relative inline-flex items-center gap-1 bg-[var(--surface-pill)] border border-[var(--border-default)] rounded-full pl-1.5 pr-2 py-0.5 text-[9px] font-semibold text-[var(--text-secondary)]">
                <Link2 size={9} />
                {chip.count}
                <span className={cn("w-1.5 h-1.5 rounded-full", INTERNAL_STATUS_DOT[chip.status] || "bg-[var(--text-info)]")} />
        ...
        <p className="font-bold text-[var(--text-primary)] text-sm italic truncate">{t.title}</p>
        <p className="text-[10px] text-[var(--text-tertiary)] font-medium mt-0.5">
          {company?.name || queues.find((q: any) => q.id === t.queueId)?.name || "Sem fila"}
        </p>
  ...
  return (
    <div className="space-y-8">
      ...
      {viewMode === "kanban" ? (
        ...
        <>
          {loading ? (
            <div className="flex flex-col items-center gap-4 text-[var(--text-tertiary)] py-20">
            ...
            <div className="flex flex-col items-center gap-2 text-[var(--text-tertiary)] py-20">
              <FileText size={48} className="opacity-20" />
              <p className="text-sm font-bold">Nenhum chamado encontrado com esses filtros</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {visibleTickets.map((t) => renderTicketCard(t))}
            </div>
          )}
          {!loading && totalPages > 1 && (
            <div className="flex items-center justify-center gap-1.5" aria-label="Paginação de chamados">
              <button
                type="button"
...
// Kanban de Chamados — mesmo padrão de arrastar-e-soltar do board de
// Tickets Internos (@dnd-kit), colunas vindas do cadastro de Status
// (Configurações > Geral), arrastar um card muda o status via PATCH em lote.
function TicketKanbanBoard({
  ...
  tickets: Ticket[];
  statuses: TicketKanbanStatusMeta[];
  loading: boolean;
  ...
  return (
    <DndContext sensors={sensors} onDragStart={handleDragStart} onDragEnd={handleDragEnd}>
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 items-start">
        {columns.map((col) => (
          <TicketKanbanColumn key={col.value} col={col} onEdit={onEdit} />
        ))}
      </div>
      <DragOverlay dropAnimation={{ duration: 180, easing: 'cubic-bezier(0.18, 0.67, 0.6, 1.22)' }}>
        {activeTicket && <TicketKanbanCard ticket={activeTicket} onEdit={() => {}} dragging />}
      </DragOverlay>
    </DndContext>
  );
}

function TicketKanbanColumn({ col, onEdit }: { col: TicketKanbanStatusMeta & { tickets: Ticket[] }; onEdit: (t: Ticket) => void }) {
  ...
  return (
    <div
      ...
      className={cn(
        ...
        isOver ? "bg-[var(--surface-pill)] ring-2 ring-[var(--accent)]/30" : "bg-[var(--surface-card)]"
      )}
    ...
    >
      ...
      <div className="p-3 space-y-2.5 min-h-[120px]">
        {col.tickets.length === 0 ? (
          <div className={cn("text-center py-8 rounded-xl border-2 border-dashed transition-colors", isOver ? "border-[var(--accent)]/40" : "border-[var(--border-default)]")}>
            <p className="text-[10px] text-[var(--text-tertiary)] font-medium uppercase tracking-wide">Arraste um card aqui</p>
          </div>
        ) : (
          col.tickets.map((ticket) => (
            <DraggableTicketKanbanCard key={ticket.id} ticket={ticket} onEdit={onEdit} />
          ))
        )}
      </div>
    </div>
...
function DraggableTicketKanbanCard({ ticket, onEdit }: { ticket: Ticket; onEdit: (t: Ticket) => void }) {
  ...
  return (
    ...
    >
      <TicketKanbanCard ticket={ticket} onEdit={onEdit} />
    </div>
  );
}

function TicketKanbanCard({ ticket, onEdit, dragging = false }: { ticket: Ticket; onEdit: (t: Ticket) => void; dragging?: boolean }) {
  const priorityLabels = ["Baixa", "Média", "Alta", "Urgente"];
  ...
  return (
    <div
      ...
      className={cn(
        ...
        dragging ? "shadow-2xl rotate-2 cursor-grabbing" : "hover:shadow-md hover:border-[var(--accent)]/40 cursor-grab active:cursor-grabbing"
      )}
    >
