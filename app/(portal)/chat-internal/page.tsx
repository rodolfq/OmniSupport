      ...
      <div className="w-[350px] border-r border-slate-100 flex flex-col bg-slate-50/30">
        <div className="p-6 space-y-6">
          <div className="flex items-center justify-between">
            <h1 className="text-2xl font-black text-slate-800 tracking-tight">Chat Interno</h1>
            <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="p-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-lg shadow-indigo-100 shrink-0"
              >
                <Plus size={20} />
              </button>
            </div>
          ...
          <div className="relative">
            ...
            <input 
              ...
              className="w-full bg-white border border-slate-200 rounded-2xl pl-12 pr-4 py-3 text-sm font-medium outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto px-4 pb-6 space-y-6">
          {findChatsWithUserId && (
            <div className="mb-2 p-3 bg-indigo-50 border border-indigo-100 rounded-2xl flex items-center justify-between">
              <div className="flex items-center gap-2">
              ...
              <button 
                ...
                className="p-1 hover:bg-indigo-200/50 rounded-lg text-indigo-600 transition-colors"
              >
                 <X size={14} />
              </button>
            </div>
          ...
          {filteredRooms.length > 0 && (
            <div className="space-y-2">
              <p className="px-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Conversas</p>
                ...
                return (
                  <div 
                    ...
                    className={cn(
                      ...
                      isActive ? "bg-white shadow-xl shadow-slate-100/50 border border-slate-100" : "hover:bg-white/50",
                      room.readLaterBy?.includes(currentUser?.id || '') && "border-2 border-indigo-200"
                    )}
                  >
                    <div className={cn(
                      "w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black overflow-hidden bg-slate-200",
                      !avatar && (room.type === 'group' ? "bg-indigo-500" : "bg-emerald-500")
                    )}>
                      {avatar ? (
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-center mb-0.5 gap-2">
                        <div className="flex items-center gap-1.5 min-w-0 flex-1">
                           <span className="text-sm font-black text-slate-800 truncate">{room.name}</span>
                        </div>
                        <div className="flex items-center gap-2 shrink-0">
                          {room.mutedBy?.includes(currentUser?.id || '') && <BellOff size={10} className="text-slate-300" />}
                          {isPinned && <Pin size={12} className="text-indigo-500 fill-indigo-500 rotate-45 shrink-0" />}
          ...
          {filteredUsers.length > 0 && (
            <div className="space-y-2">
              <p className="px-4 text-[10px] font-black uppercase text-slate-400 tracking-widest">Contatos</p>
              {filteredUsers.map((user, uIdx) => (
                <button 
                  key={`contact-${user.id}-${uIdx}`}
                  onClick={() => startDirectChat(user)}
                  className="w-full flex items-center gap-4 p-4 rounded-3xl transition-all text-left hover:bg-white/50 group"
                >
                  <div className={cn(
                    "w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black overflow-hidden bg-slate-200",
                    !user.avatarUrl && "bg-emerald-500"
                  )}>
                    {user.avatarUrl ? (
                  ...
                  <div className="flex-1 min-w-0">
                    <span className="text-sm font-black text-slate-800 truncate block">{user.name}</span>
      ...
      <div className="flex-1 flex flex-col bg-white">
        {selectedRoom ? (
          <>
            ...
            <div className="px-8 py-5 border-b border-slate-100 flex items-center justify-between bg-white/80 backdrop-blur-xl sticky top-0 z-10">
              <div className="flex items-center gap-4">
                <div className={cn(
                  "w-12 h-12 rounded-2xl flex items-center justify-center text-white font-black shadow-lg overflow-hidden bg-slate-200",
                  selectedRoom.type === 'group' ? "bg-indigo-500 shadow-indigo-100" : "bg-emerald-500 shadow-emerald-100"
                )}>
                ...
                <div>
                  <h2 className="text-lg font-black text-slate-800 tracking-tight">{selectedRoom.name}</h2>
            ...
            {selectedRoom.pinnedMessageIds && selectedRoom.pinnedMessageIds.length > 0 && !showPinnedOnly && (
               <div className="bg-indigo-50/50 border-b border-indigo-100 px-8 py-2 flex items-center justify-between">
                  ...
                  <button 
                    ...
                    className="text-[10px] font-black uppercase text-indigo-600 hover:underline shrink-0"
                  >
                    Ver Tudo
                  </button>
               </div>
                ...
                return (
                  ...
                  >
                    {!isMine && avatarSize !== 'none' && (
                      <div className={cn("rounded-2xl bg-slate-200 shrink-0 overflow-hidden mt-6 shadow-sm", avatarSizeClass)}>
                    ...
                    <div className={cn("flex flex-col", isMine ? "items-end" : "items-start")}>
                      ...
                      <div className={cn(
                        ...
                        isPinned && "ring-2 ring-indigo-500/20"
                      )}>
                        {isPinned && !showPinnedOnly && (
                           <div className="absolute -top-2 -right-2 w-5 h-5 bg-indigo-600 rounded-full flex items-center justify-center text-white shadow-lg border border-white">
                        ...
                        {repliedMessage && !msg.isDeleted && (
                           <div className={cn(
                             ...
                             isMine ? "border-white/40" : "border-indigo-500/40"
                           )}>
                            ...
                            {msg.type === 'file' && (
                              <div className={cn(
                                ...
                                <div className={cn(
                                  ...
                                  isMine ? "bg-white/20" : "bg-white border border-slate-100"
                                )}>
                                  <FileText size={20} />
                                </div>
                                ...
                                <button className={cn(
                                  ...
                                  isMine ? "hover:bg-white/20" : "hover:bg-white"
                                )}>
                                  <Download size={16} />
                                </button>
                              </div>
                        ...
                        <div className={cn(
                          ...
                          {isMine && !msg.isDeleted && (
                            <button 
                              ...
                              className="text-[9px] font-black uppercase text-red-400 hover:text-red-600 mr-2"
                            >
                              Excluir
                            </button>
                          )}
                          {!msg.isDeleted && (
                             <>
                                <button 
                                  ...
                                  className="text-[9px] font-black uppercase text-indigo-400 hover:text-indigo-600 mr-2"
                                >
                                  Responder
                                </button>
                                <button 
                                  ...
                                  className={cn(
                                    ...
                                    isPinned ? "text-indigo-600" : "text-slate-400 hover:text-slate-600"
                                  )}
                                >
                          ...
                          {isMine && (
                            msg.readBy && msg.readBy.length > (selectedRoom.type === 'group' ? 1 : 1) ? (
                              <CheckCheck size={12} className={cn("text-indigo-400", (selectedRoom.type === 'direct' ? msg.readBy?.some(id => id !== currentUser?.id) : (msg.readBy?.length || 0) >= (selectedRoom.memberIds?.length || 0)) && "text-blue-500")} />
                            ) : (
            ...
            <div className="p-6 bg-white border-t border-slate-100">
              <div className="max-w-4xl mx-auto relative">
                ...
                <AnimatePresence>
                  ...
                  {showStickerPicker && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      className="absolute bottom-full mb-4 left-0 bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl z-20 w-[300px]"
                    >
                        <div className="flex items-center justify-between mb-4">
                           <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Stickers</p>
                           <button 
                              ...
                              <div className="grid grid-cols-3 gap-3">
                                 {(currentUser.chatPreferences.personalStickers || []).map((url, idx) => (
                                   <button 
                                     ...
                                     className="aspect-square rounded-xl overflow-hidden hover:scale-110 transition-all border border-slate-100 relative group"
                                   >
                  ...
                  {showGifSearch && (
                    <motion.div 
                      ...
                      animate={{ opacity: 1, y: 0, scale: 1 }}
                      exit={{ opacity: 0, y: 10, scale: 0.95 }}
                      className="absolute bottom-full mb-4 left-0 bg-white border border-slate-200 rounded-3xl p-6 shadow-2xl z-20 min-w-[320px]"
                    >
                       <div className="flex items-center gap-2 mb-4 bg-slate-100 rounded-xl p-2 pr-4">
                       ...
                       <div className="grid grid-cols-2 gap-3 max-h-[300px] overflow-y-auto pr-2">
                          {[
                            ...
                            <button 
                              ...
                              className="rounded-lg overflow-hidden h-24 hover:opacity-80 transition-all bg-slate-100"
                            >
                ...
                <div className="flex items-center gap-4 bg-slate-50 border border-slate-100 rounded-[2rem] p-3 pl-6 pr-3 shadow-inner">
                  <div className="flex items-center gap-2">
                    <button 
                      onClick={() => {
                      ...
                      className={cn("p-2 rounded-full transition-all", showEmojiPicker ? "bg-indigo-100 text-indigo-600" : "text-slate-400 hover:text-indigo-600")}
                    >
                      <Smile size={24} />
                    </button>
                    <button 
                      ...
                      className={cn("p-2 rounded-full transition-all", showStickerPicker ? "bg-indigo-100 text-indigo-600" : "text-slate-400 hover:text-indigo-600")}
                    >
                      <ImageIcon size={24} />
                    </button>
                    <button 
                      ...
                      className={cn("px-2 py-1 rounded-lg transition-all font-black text-xs", showGifSearch ? "bg-indigo-100 text-indigo-600" : "text-slate-400 hover:text-indigo-600")}
                    >
                      GIF
                    </button>
                    <button 
          ...
          <div className="flex-1 flex flex-col items-center justify-center text-center p-12 bg-slate-50/30">
            <div className="w-24 h-24 bg-white rounded-[2rem] flex items-center justify-center shadow-2xl shadow-indigo-100 mb-8 border border-slate-100">
               <MessageCircle className="text-indigo-500" size={48} />
            </div>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-6">
             ...
             >
                <div className="p-8 border-b border-slate-100 flex items-center justify-between">
                   ...
                   <button onClick={() => setIsCreatingGroup(false)} className="p-2 hover:bg-slate-100 rounded-xl transition-all">
                ...
                <div className="p-8 space-y-6">
                   <div className="flex justify-center">
                      <div 
                        ...
                        className="w-24 h-24 rounded-[2rem] bg-slate-50 border-2 border-dashed border-slate-200 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-500 hover:bg-slate-100 transition-all overflow-hidden relative group"
                      >
                         {newGroupImage ? (
                   ...
                   <div className="space-y-2">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Nome do Grupo</label>
                      <input 
                        type="text"
                        value={newGroupName}
                        onChange={(e) => setNewGroupName(e.target.value)}
                        placeholder="Ex: Time de Suporte, Infraestrutura..."
                        className="w-full bg-slate-50 border border-slate-200 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/5 transition-all"
                      />
                   </div>

                   <div className="space-y-3">
                      <label className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Membros (Mais de 1 vira grupo)</label>
                         ...
                         {allUsers.map((user, uIdx) => (
                           <button 
                             ...
                             className={cn(
                               ...
                               selectedMembers.includes(user.id) ? "bg-indigo-50" : "hover:bg-slate-50"
                             )}
                           >
                              <div className={cn(
                                "w-10 h-10 rounded-xl flex items-center justify-center text-white font-black",
                                selectedMembers.includes(user.id) ? "bg-indigo-500" : "bg-slate-400"
                              )}>
                                {user.name.charAt(0)}
                              </div>
                   ...
                   <button 
                     ...
                     className="w-full bg-indigo-600 text-white rounded-2xl py-4 font-black transition-all hover:bg-indigo-700 shadow-xl shadow-indigo-100 disabled:opacity-50"
                   >
                     Criar Conversa
                   </button>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-8 space-y-8">
                ...
                <div className="flex items-center gap-6">
                   <div 
                     ...
                     className="w-24 h-24 rounded-[2rem] bg-slate-100 border-2 border-slate-200 flex items-center justify-center cursor-pointer hover:border-indigo-500 transition-all overflow-hidden shrink-0 group relative"
                   >
                   ...
                   <div className="flex-1 space-y-2">
                       ...
                       <input 
                         ...
                         className="w-full bg-slate-50 border border-slate-100 rounded-2xl px-5 py-3 text-sm font-bold outline-none focus:border-indigo-500 transition-all"
                       />
                   </div>
                </div>
                ...
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    ...
                    <button 
                      ...
                      className="flex items-center gap-2 text-[10px] font-black uppercase text-indigo-600 hover:text-indigo-800 tracking-widest"
                    >
                    ...
                    >
                       ...
                       <div className="max-h-[200px] overflow-y-auto space-y-1">
                          {allUsers.filter(u => !selectedRoom.memberIds?.includes(u.id)).map((user, idx) => (
                            ...
                            >
                               <div className="w-8 h-8 rounded-lg bg-indigo-500 flex items-center justify-center text-white text-xs font-black">
                                  {user.name.charAt(0)}
                               </div>
                               <div>
                               ...
                               <Plus size={14} className="ml-auto text-indigo-500" />
                            </button>
                          ))}
                       </div>
                    </motion.div>
                  ...
                  <div className="space-y-2">
                    {(selectedRoom.memberIds || []).map((userId, idx) => {
                      ...
                      if (!user) return null;
                      return (
                        <div key={`member-${userId || idx}`} className="flex items-center gap-4 p-3 rounded-3xl hover:bg-slate-50 transition-all group/member">
                           <div className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center overflow-hidden">
                              {user.avatarUrl ? <img src={user.avatarUrl} className="w-full h-full object-cover" /> : <span className="font-black text-slate-400">{user.name.charAt(0)}</span>}
                           </div>
                           <div className="flex-1">
                              <p className="text-sm font-black text-slate-800">{user.name}{userId === currentUser?.id && ' (Sua conta)'}</p>
                              <p className="text-[10px] font-bold text-slate-400 uppercase tracking-tighter">{user.role}</p>
                           </div>
                           {userId !== currentUser?.id && (
                             <button 
                               onClick={() => handleRemoveMember(userId)}
                               className="p-2 text-slate-300 hover:text-rose-500 opacity-0 group-hover/member:opacity-100 transition-all"
                ...
                <div className="pt-4">
                  <button 
                    ...
                    className="w-full flex items-center justify-center gap-2 py-4 text-rose-500 font-black text-sm uppercase tracking-widest hover:bg-rose-50 rounded-2xl transition-all"
                  >
                    ...
                    {['indigo', 'emerald', 'blue', 'rose', 'amber', 'slate', 'violet'].map(color => (
                       <button
                         ...
                         className={cn(
                           ...
                           bubbleColor === color ? "border-indigo-500 scale-110 shadow-lg" : "border-transparent opacity-80"
                         )}
            ...
            >
               <div className="p-8 border-b border-slate-100 flex items-center justify-between shrink-0 bg-white z-10">
                  <div className="flex items-center gap-3">
                     ...
                     <h3 className="text-xl font-black text-slate-800 tracking-tight">Criar Sticker</h3>
                  </div>
                  <button onClick={() => setIsCroppingSticker(false)} className="p-2 hover:bg-slate-100 rounded-xl transition-all">
                    <X size={20} className="text-slate-400" />
                  </button>
               </div>
                    ...
                    <div className="absolute bottom-6 left-1/2 -translate-x-1/2 z-20">
                      <div className="bg-amber-100/90 backdrop-blur-md px-4 py-2 rounded-xl border border-amber-200 flex items-center gap-3 shadow-lg">
                        ...
                        <button 
                          ...
                          className="px-3 py-1 bg-amber-600 text-white rounded-lg text-[9px] font-black uppercase shadow-sm hover:bg-amber-700 transition-all"
                        >
               ...
               <div className="p-8 space-y-6 shrink-0 bg-white">
                  <div className="space-y-2">
                  ...
                  <div className="flex gap-4">
                     <button 
                       onClick={() => setIsCroppingSticker(false)}
                       className="flex-1 px-6 py-4 rounded-2xl font-black text-sm uppercase tracking-widest text-slate-500 hover:bg-slate-50 transition-all"
                     >
                       Cancelar
                     </button>
                     <button 
                       ...
                       className="flex-1 bg-indigo-600 text-white rounded-2xl py-4 font-black transition-all hover:bg-indigo-700 shadow-xl shadow-indigo-100"
                     >
                       Salvar Sticker
                     </button>
                  </div>
        ...
        {stickerContextMenu && (
          ...
          >
            <motion.div 
               ...
               className="absolute bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 w-48 overflow-hidden"
            ...
            >
               <button 
                 ...
                 className="w-full flex items-center gap-3 px-4 py-3 hover:bg-rose-50 text-rose-600 transition-colors text-sm font-bold"
               >
      ...
      <AnimatePresence>
        {contextMenu && (
          <motion.div
            ...
            animate={{ opacity: 1, scale: 1 }}
            exit={{ opacity: 0, scale: 0.95 }}
            className="fixed z-[100] w-64 bg-white rounded-2xl shadow-2xl border border-slate-100 overflow-hidden py-2"
               ...
               return (
                 <>
                   ...
                   <button 
                     ...
                     className="w-full flex items-center gap-3 px-4 py-3 hover:bg-slate-50 text-slate-700 transition-colors text-sm font-bold"
                   >
                      {isPinned ? <PinOff size={16} className="text-slate-400" /> : <Pin size={16} className="text-slate-400" />}
                      {isPinned ? 'Desafixar' : 'Fixar'}
                   </button>
                   ...
                   <button 
                     ...
                     className={cn(
                       "w-full flex items-center gap-3 px-4 py-3 hover:bg-rose-50 text-rose-600 transition-colors text-sm font-bold",
                       room.hiddenBy?.includes(currentUser?.id || '') && "text-indigo-600 hover:bg-indigo-50"
                     )}
                   >
